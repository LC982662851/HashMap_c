...
HashMap * create_MyHashMap(int (*hash)(char *key),int (*equal)(char *k1,char *k2)){
    ...
    hashMap->initialCapacity=DEFAULT_INITIAL_CAPACITY;
...
//删
int  remove_Node(HashMap* hashMap,char * const k,void * const v){
    int index=hashcode(k,hashMap);
    ...
    if(hashMap->table[index].data.key==k){
        ...
        HashNode *  hm = &(hashMap->table[index].next);
        while(hm!=NULL){
            if(hm->data.key==k)
            {
                HashMap* hmtmp=hm->next;
                hm->next=hmtmp->table->next;
                free(hmtmp);
                break;
            }
            else
